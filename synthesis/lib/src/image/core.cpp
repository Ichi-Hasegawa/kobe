...
panorama::windowing_HU(const typename itk::Image<PixelType, 3>::Pointer &img) {
    // Define region for the 3D image
    ...
    for (std::size_t z = 0; z < size[2]; ++z) {
        for (std::size_t y = 0; y < size[1]; ++y) {
            for (std::size_t x = 0; x < size[0]; ++x) {
                ...
                idx[2] = static_cast<long>(z);

                auto pixel_value = img->GetPixel(idx);
                if (pixel_value > HU_MAX) {
                    pixel_value = HU_MAX;
                } else if (pixel_value < HU_MIN) {
                    pixel_value = HU_MIN;
                }
                img3d->SetPixel(idx, pixel_value);
            }
        }
    }
...
panorama::extract_slices(
    ...
    //std::cout << "Before condition check: As = " << As << ", Ae = " << Ae << ", Slice Number = " << slice_num << std::endl;
    ...
    //std::cout << "After adjustment: As = " << As << ", Ae = " << Ae << std::endl;
...
panorama::rotate_ct_image(
    ...
    // Set the rotation matrix based on the specified axis
    if (axis == 'x') {
    ...
    // Get the center of the image and set it as the rotation center
    ...
    // 2. Set up the resampling filter
    ...
    // Configure output image properties (size, spacing, origin, direction)
    ...
    // Set the default pixel value for areas outside the input image (e.g., 0)
    resample->SetDefaultPixelValue(0);

    // 3. Execute the resampling and obtain the rotated image
    resample->Update();
...
#define PIXEL_TYPE_IMAGE(T) \
    template itk::Image<T, 3>::Pointer panorama::windowing_HU<T>(const typename itk::Image<T, 3>::Pointer &img); \
