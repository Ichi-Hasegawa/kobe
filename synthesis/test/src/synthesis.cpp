...
// template <typename PixelType>
// typename itk::Image<PixelType, 2>::Pointer
// poemi::sharpen_panoramic_image(
//     const typename itk::Image<PixelType, 2>::Pointer &img
// ) {
//     using ImageType = itk::Image<PixelType, 2>;
//     using GaussianFilter = itk::DiscreteGaussianImageFilter<ImageType, ImageType>;
//     using SubtractFilter = itk::SubtractImageFilter<ImageType>;
//     using MultiplyFilter = itk::MultiplyImageFilter<ImageType>;
//     using AddFilter = itk::AddImageFilter<ImageType>;

//     std::vector<double> sigmas = {2.4, 4.8, 19.2};
//     std::vector<double> alphas = {1.0, 1.0, 1.5, 1.5};

//     // α0 * I0
//     auto output = MultiplyFilter::New();
//     output->SetInput1(img);
//     output->SetConstant(alphas[0]);
//     output->Update();

//     typename ImageType::Pointer result = output->GetOutput();

//     for (size_t i = 0; i < sigmas.size(); ++i) {
//         // Gaussian blur
//         auto blur = GaussianFilter::New();
//         blur->SetInput(img);
//         blur->SetVariance(sigmas[i] * sigmas[i]); // σ²
//         blur->Update();

//         // I0 - Gi
//         auto diff = SubtractFilter::New();
//         diff->SetInput1(img);
//         diff->SetInput2(blur->GetOutput());
//         diff->Update();

//         // αi * (I0 - Gi)
//         auto amp = MultiplyFilter::New();
//         amp->SetInput(diff->GetOutput());
//         amp->SetConstant(alphas[i + 1]);
//         amp->Update();

//         // Add to result
//         auto add = AddFilter::New();
//         add->SetInput1(result);
//         add->SetInput2(amp->GetOutput());
//         add->Update();
...
template <typename PixelType>
typename itk::Image<PixelType, 2>::Pointer
poemi::sharpen_panoramic_image(const typename itk::Image<PixelType, 2>::Pointer& img)
{
    using ImageType = itk::Image<PixelType, 2>;
    ...
    gaussian->SetVariance(sigma * sigma);  // ITKはσ²で指定
    gaussian->SetUseImageSpacing(false);   // spacing無視（ピクセル単位で指定）
    gaussian->Update();

    // Step 2: I0 - G(I0)
    auto edge = SubtractFilterType::New();
    edge->SetInput1(img);
    edge->SetInput2(gaussian->GetOutput());
    edge->Update();

    // Step 3: (1 - α)(I0 - G(I0))
    auto scaled_edge = MultiplyFilterType::New();
    scaled_edge->SetInput(edge->GetOutput());
    scaled_edge->SetConstant(1.0 - alpha);
    scaled_edge->Update();

    // Step 4: α * I0
    auto scaled_img = MultiplyFilterType::New();
    scaled_img->SetInput(img);
    scaled_img->SetConstant(alpha);
    scaled_img->Update();

    // Step 5: I = α * I0 + (1 - α) * (I0 - G(I0))
    auto enhanced = AddFilterType::New();
    enhanced->SetInput1(scaled_img->GetOutput());
    enhanced->SetInput2(scaled_edge->GetOutput());
    enhanced->Update();

    return enhanced->GetOutput();
}
...
#define PIXEL_TYPE_SYNTHESIS(T) \
    ...
    template itk::Image<T, 2>::Pointer poemi::sharpen_panoramic_image<T>(const typename itk::Image<T, 2>::Pointer &img);
